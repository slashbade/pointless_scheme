...
\section{Points of Locales}

\begin{definition}[Completely Prime Filter]
  \label{def:completely-prime-filter}
  \lean{CompletelyPrimeFilter}
  \leanok
  A \emph{completely prime filter} on a frame $L$ is a filter $F \subseteq L$ such that:
  \begin{enumerate}
    \item $\top \in F$ (contains the top element)
    \item $F$ is upward closed: if $u \in F$ and $u \leq v$, then $v \in F$
    \item $F$ is closed under meets: if $u, v \in F$, then $u \land v \in F$
    \item $F$ is completely prime: for any family $(u_i)_{i \in I}$, if $\bigvee_i u_i \in F$ then $\exists i, u_i \in F$
  \end{enumerate}

  Completely prime filters correspond to ``points'' of the locale in pointless topology.
\end{definition}

\begin{theorem}[Prime Ideals Correspond to Filters]
  \label{thm:prime-ideal-to-filter}
  \uses{def:completely-prime-filter, def:radical-ideal}
  \lean{Rad.filterOfPrime}
  \leanok
  For a prime ideal $\mathfrak{p}$ of a commutative ring $R$, the set
  $$F_{\mathfrak{p}} := \{I \in \mathrm{Rad}(R) : \exists f \in I, f \notin \mathfrak{p}\}$$
  forms a completely prime filter on $\mathrm{Rad}(R)$.

  This establishes the correspondence: points of $\mathrm{Spec}\, R$ $\leftrightarrow$ prime ideals of $R$.
\end{theorem}
...
\begin{definition}[Locally Ringed Locale]
  ...
  \uses{def:locale, def:sheaf-locale, def:completely-prime-filter}
  \lean{LocallyRingedLocale}
  \leanok
  A \emph{locally ringed locale} is a triple $(X, \mathcal{O}_X, \text{stalks\_local})$ where:
  \begin{enumerate}
    \item $X$ is a locale (a frame)
    \item $\mathcal{O}_X$ is a presheaf of rings on $X$
    \item For every completely prime filter $F$ on $X$, the stalk $\mathcal{O}_{X,F}$ is a local ring
  \end{enumerate}
\end{definition}
...
\begin{definition}[Stalk at a Prime Ideal]
  \label{def:stalk-at-prime}
  \uses{def:stalk-locale, thm:prime-ideal-to-filter}
  \lean{Spec.stalkAtPrime}
  \leanok
  For a prime ideal $\mathfrak{p}$ of $R$, the stalk of the structure sheaf at $\mathfrak{p}$ is the localization:
  $$\mathcal{O}_{\mathrm{Spec}\, R, \mathfrak{p}} := R_{\mathfrak{p}} = \mathrm{Localization.AtPrime}\, \mathfrak{p}$$
  This is a local ring with maximal ideal $\mathfrak{p} R_{\mathfrak{p}}$.
\end{definition}

\begin{theorem}[Stalk at Prime is Local]
  \label{thm:stalk-at-prime-local}
  \uses{def:stalk-at-prime}
  \lean{Spec.stalkAtPrime.isLocalRing}
  \leanok
  For any prime ideal $\mathfrak{p}$, the stalk $R_{\mathfrak{p}}$ is a local ring.
  This is provided by \texttt{Localization.AtPrime.isLocalRing} from Mathlib.
\end{theorem}
...
\begin{definition}[Structure Presheaf of Affine Scheme]
  \label{def:structure-presheaf-affine}
  \uses{def:affine-scheme, def:structure-sheaf-radical-ideals}
  \lean{AffineScheme.structurePresheaf}
  \leanok
  For an affine scheme $\mathrm{Spec}\, R$, the structure presheaf is defined as:
  $$\mathcal{O}_{\mathrm{Spec}\, R}(I) := \mathrm{Spec.sheafExtend}\, I$$
